...
public class SDG {
	...
	public DotGraph toDotGraph() {
		...
		for (HashMutablePDG pdg : connections.keySet()) {
			...
			//Add node instead of label of cluster, and use this to add edges between the cfgSubGraph clusters
								...
								if (useValueBoxes != null) {
									for (ValueBox vb : useValueBoxes) {
										...
										if (v instanceof JimpleLocal) {
											...
											if (edgeToUnit != null)
												subGraph.drawEdge(String.valueOf(edgeToUnit.hashCode()), String.valueOf(u.hashCode())).setLabel("D_"+varName);;
										}
									}
								}
							}
		...
		for (List<SdgEdge> edges : connections.values()) {
			for (SdgEdge edge : edges) {
				if (edge.type == EdgeType.RET) {
					...
					//dotEdge.setAttribute("ltail", "cluster_"+edge.pdgFrom.hashCode()); //Head of edge should point to SDG				
				} else {
					DotGraphEdge dotEdge = dot.drawEdge(String.valueOf(edge.from.hashCode()),
							"StartNode" + edge.pdgTo.GetStartNode().hashCode());
					dotEdge.setAttribute("lhead", "cluster_"+edge.pdgTo.hashCode()); //Head of edge should point to SDG
					dotEdge.setLabel(edge.type.toString());
