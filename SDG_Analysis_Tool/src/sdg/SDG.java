					...
					if (!visited.contains(succ)) {
						...
						if (succ.getNode() instanceof Block) {
							...
							while (it.hasNext()) {
								...
								// Add edge on variable write
								List<ValueBox> defValueBoxes = u.getDefBoxes();
								if (defValueBoxes != null) {
									for (ValueBox dvb : defValueBoxes) {
										Value v = dvb.getValue();
										 if (v instanceof FieldRef) {
											SootField sf = ((FieldRef) v).getField();
											if (sootFields.contains(sf)) {
												dot.drawEdge(String.valueOf(u.hashCode()), String.valueOf("field" + sf.hashCode())).setLabel("D_" + sf.toString());
											}
										 }
									}
								}
						...
						//Add successor to worklist, so we continue to iterate the PDG
						worklist.add(succ);
					}
		...
		for (List<SdgEdge> edges : connections.values()) {
			for (SdgEdge edge : edges) {
				if (edge.type == EdgeType.RET) {
					...
					//dotEdge.setAttribute("lhead", "cluster_" + edge.pdgTo.hashCode()); // Head of edge should point to PDG
					dotEdge.setLabel(edge.type.toString());
